...
if ($tglAwal != null && $tglAkhir != null) {
    ...
    LEFT JOIN 
        ...
        WHERE tb_permintaan.created_at BETWEEN '$tglAwal' AND DATE_ADD('$tglAkhir', INTERVAL 1 DAY) ORDER BY tb_permintaan.id ASC");
} else {
    $tanggalSaatIni = date(' F Y');
    $query = mysqli_query($conn, "SELECT 
    tb_permintaan.created_at as created_at,
...
while ($data = mysqli_fetch_array($query)) {
    ...
    <tr>
    <td>'.$no++.'</td>
    <td>'.ucwords($data['nama_teknisi']).'</td>
    <td>'.$data['nama_barang'].'</td>
    <td>'.$data['jumlah_barang'].'</td>
    <td>'.ucwords($data['nama_staff']).'</td>
    <td>'.ucwords($data['nama_operator']).'</td>
    <td>'.$data['status'].'</td>
...
$pdf->writeHTMLCell(0, 0, 5, 40, $tabel, 0, 1, 0, true, '', true);
